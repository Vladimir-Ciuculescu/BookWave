...
import { useFetchAutoGeneratedPlaylists } from "hooks/playlists.queries";
...
const HomeScreen: React.FC<any> = () => {
  ...
  const { data: autoGeneratedPlaylists, isLoading: areAutoGeneratedPlaylistsLoading } = useFetchAutoGeneratedPlaylists();
        ...
        >
          {!latestAudios.uploads.length ? (
            <BWView column alignItems="center" justifyContent="center" style={styles.noDataContainer} gap={25}>
              <NoResultsFound width="100%" height={200} />
  ...
  const renderAutoGeneratedPlaylists = () => {
    if (areAutoGeneratedPlaylistsLoading) {
      return Array.of(5).map((_, index) => <Skeleton colorMode="dark" width={180} height={180} key={index} />);
    } else {
      return (
        <ScrollView
          showsHorizontalScrollIndicator={false}
          horizontal
          contentContainerStyle={[styles.list, !autoGeneratedPlaylists.playlists.length ? { width: "100%" } : null]}
        >
          {!autoGeneratedPlaylists.playlists.length ? (
            <BWView column alignItems="center" justifyContent="center" style={styles.noDataContainer} gap={25}>
              <NoResultsFound width="100%" height={200} />
              <Text style={styles.notFoundTitle}>No uploads found</Text>
            </BWView>
          ) : (
            autoGeneratedPlaylists.playlists.map((playlist: PlayListElement, index: number) => index < 5 && <PlayListItem key={index} playlist={playlist} />)
          )}
        </ScrollView>
      );
    }
          ...
          <BWView column gap={50} style={styles.screenContainer}>
            <BWView column gap={25}>
              <BWView row justifyContent="space-between">
                ...
                {latestAudios && latestAudios.uploads.length > 3 && <BWButton onPress={goToLatestAudios} title="See all" link labelStyle={styles.sectionBtn} />}
              </BWView>
              {renderLatestUploads()}
            </BWView>
            <BWView column gap={25}>
              <BWView row justifyContent="space-between">
                ...
                {recommendedAudios && recommendedAudios.audios.length > 3 && (
            ...
            <BWView column gap={25}>
              <BWView row justifyContent="space-between">
                <Text style={styles.sectionTitle}>Auto generated Playlists</Text>
                {autoGeneratedPlaylists && autoGeneratedPlaylists.playlists.length > 3 && (
                  <BWButton onPress={goToRecommendedAudios} title="See all" link labelStyle={styles.sectionBtn} />
                )}
              </BWView>
              {renderAutoGeneratedPlaylists()}
            </BWView>
          </BWView>
        </ScrollView>
